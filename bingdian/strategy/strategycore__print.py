...
class StrategyCore(BfTraderClient):
   ...
   #下单函数
    ...
    def SP_BK(self,price,volume): #空翻多, 返回三个单号---平今 平昨 开仓
        ...
        print u'冻结',toltal_pos,'\t',self.frozenPos
    ...
    def OnOrder(self, orderData):
        ...
        if orderData.symbol == self.symbol and isMyData:
            ...
            if orderData.status >1:
                if orderData.offset == OFFSET_CLOSETODAY or orderData.offset == OFFSET_CLOSE :
                    print u'\nOrder回报:',TR_Direction[orderData.direction],TR_Offset[orderData.offset],orderData.price, \
                                orderData.tradedVolume,'/',orderData.totalVolume,ORDER_STATUS[orderData.status],orderData.bfOrderId
                    freeValume=orderData.totalVolume-orderData.tradedVolume if orderData.status == STATUS_CANCELLED else orderData.tradedVolume
                    if orderData.status == STATUS_CANCELLED: print u'撤单成功成功'
                    print '\t',self.frozenPos,u'\n解冻',freeValume,
                    self.frozenPos[orderData.direction][orderData.offset] -= freeValume
                    print '\t',self.frozenPos
